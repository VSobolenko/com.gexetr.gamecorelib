...
{
    private const string SceneLoaderName = "Scene Loader";
    
    private EditorBuildSettingsScene[] _scenes;

    [MenuItem(GameData.EditorName + "/" + SceneLoaderName)]
    private static void OpenWindow() => ShowWindow<SceneSwitcher>();

    protected static T ShowWindow<T>(string title = SceneLoaderName, Action<T> startupConfigure = null)
    ...
    private static void DrawStartedSceneSelection()
    {
        ...
        if (GUILayout.Button(new GUIContent(text), buttonStyle))
        {
            ...
            EditorUtility.DisplayCustomMenu(new Rect(Event.current.mousePosition, Vector2.zero), options, selectedIndex,
